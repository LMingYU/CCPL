    ...
    for epoch in range(25):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if epoch>-1:
                ...
                if epoch>-1:   
                   loss =Lx + Lconl+coef2*Lu+coef*Lpri+coef3*Lcon
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s.  ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
        ...
        if args.local_rank in [-1, 0]:
            ...
            if is_best:
                torch.save(model_to_save.state_dict(), 'FM_600_supcon_state_dict.pkl')
                if args.use_ema:
                    torch.save(ema_to_save.state_dict(), 'FM_600_supcon_ema_state_dict.pkl')
                else:
                    torch.save(None, 'FM_600_supcon_ema_state_dict.pkl')
